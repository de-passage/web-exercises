...
flip' = ap3 S (ap3 comp S (ap3 comp K S)) (Ap K K)
...
rotr = ap3 S (Ap K (Ap S S)) (ap3 comp K K)
...
rotv = ap3 S (ap3 S (Ap K comp) (ap3 comp S rev)) (Ap K K)
...
-- or x y = x T y = rev T x y
or' :: SKI (Bool' (Bool' a) -> Bool' a -> Bool' a)
or' = Ap rev true

-- xor x y = x (not y) y = x (not y) (I y) = x not I y = x not I = rotv not I
